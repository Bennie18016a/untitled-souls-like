...
public class BossStafeState : BossBaseState
{
    ...
    private Vector3 pos;
    public BossStafeState(BossStateMachine stateMachine) : base(stateMachine) { }

    public override void Enter()
    {
        _stateMachine.Animator.CrossFadeInFixedTime(LocomotionBlendTreeHash, 0.1f);

        newPos = _stateMachine.Player.transform.position + Random.insideUnitSphere * _stateMachine.StrafeDistance;
        ...
        {
            newPos = _stateMachine.transform.position + Random.insideUnitSphere * _stateMachine.StrafeDistance;
            newPos.y = _stateMachine.transform.position.y;
        }
    }
